...
class CompareTransform:
    ...
    def visit_list(
        ...
        left_type = _resolve_type(left)
        right_type = _resolve_type(right)
        if not _match_types(left_type, right_type):
            self.report_error(
                f'type mismatch expected: {left_type}, got: {right_type}',
            )
            return right
        left_len = _resolve_length(left)
        right_len = _resolve_length(right)
    ...
    def visit_dict(self, left: typing.Dict, right: typing.Any) -> typing.Dict:
        right_type = _resolve_type(right)
        if not issubclass(right_type, dict):
            self.report_error(
                f'dict expected on the right, got {right_type!r} instead'
            )
            return right
        left_len = _resolve_length(left)
        right_len = _resolve_length(right)
...
def _resolve_type(obj):
    if hasattr(obj, '__testsuite_type__'):
        return obj.__testsuite_type__
    return type(obj)
